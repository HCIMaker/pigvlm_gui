...
class DLCFramesDock(DockWidget):
    ...
    Rows populate from the current video's `filename` list. Double-clicking
    a row navigates to that frame; the row selection tracks the current
    `frame_idx` as the user steps through frames via arrow keys / seekbar.
    Non-ImageVideo projects show an empty list (harmless).
    """

    def __init__(self, main_window: QMainWindow, tab_with: Optional[QLayout] = None):
        super().__init__(
            name="DLC Image Frames",
            main_window=main_window,
            model_type=DLCFramesTableModel,
            tab_with=tab_with,
        )
        main_window.state.connect("video", self._on_video_changed)
        main_window.state.connect("frame_idx", self._on_frame_changed)

    def create_models(self) -> DLCFramesTableModel:
        self.model = self.model_type(context=self.main_window.commands)
        return self.model
    ...
    def create_tables(self) -> GenericTableView:
        ...
        def goto_frame(*args):
            ...
            frame_idx = item.get("_frame_idx")
            if video is not None and frame_idx is not None:
                self.main_window.commands.gotoVideoAndFrame(video, frame_idx)

        self.table.doubleClicked.connect(goto_frame)
        return self.table
    ...
    def _on_frame_changed(self, frame_idx):
        if frame_idx is None:
            return
        # Only relevant when model has rows (ImageVideo backend).
        if 0 <= frame_idx < len(self.model.items or []):
            self.table.selectRow(frame_idx)
