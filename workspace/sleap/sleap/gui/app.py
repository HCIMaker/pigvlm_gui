...
class MainWindow(QMainWindow):
    ...
    def setWindowTitle(self, value):
        ...
        if value is not None:
            labeler = None
            labels = self.state.get("labels", None)
            if labels is not None:
                labeler = labels.provenance.get("labeler")
            suffix = f" — {labeler}" if labeler else ""
            super(MainWindow, self).setWindowTitle(
                f"{value}{suffix} - SLEAP v{sleap.version.__version__}"
            )
    ...
    def _create_menus(self):
        ...
        add_menu_item(
            fileMenu, "new_dlc", "New DLC Project...", self.commands.newDLCProject
        )
    ...
    def _create_dock_windows(self):
        ...
        self.dlc_frames_dock = DLCFramesDock(self, tab_with=self.videos_dock)
    ...
    def updateStatusMessage(self, message: Optional[str] = None):
        ...
        if message is None:
            ...
            if current_video is not None:
                ...
                # ImageVideo backends store one filename per frame (list[str]);
                # surface the current image's name so DLC `img<NNN>.png`
                # folders are navigable by filename, not just frame index.
                vid_fn = current_video.filename
                if isinstance(vid_fn, list) and 0 <= frame_idx < len(vid_fn):
                    message += f" ({Path(vid_fn[frame_idx]).name})"
